...
unsigned int GetTokens(FILE* F, Token TokenTable[], FILE* errFile)
{
	...
	while (1)
	{
		...
		case Letter:
		{
			...
			else if (!strcmp(buf, "int_2"))	temp_type = Type;
			...
			else if (!strcmp(buf, "noteq"))		temp_type = NotEquality;
			else if (!strcmp(buf, "less"))		temp_type = Less;
			else if (!strcmp(buf, "gr"))		temp_type = Greate;
